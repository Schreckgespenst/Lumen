  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end gap-1">
        {RANGES.map((r) => (
          <button
            ...
            className={`text-xs px-2 py-1 rounded ${range === r.id ? 'bg-accent text-white' : 'text-subtle hover:text-text'}`}
          >{r.label}</button>
        ))}
      </div>
      ...
      <form onSubmit={onAdd} className="bg-card rounded-2xl border border-muted p-4 flex gap-2">
        <select value={type} onChange={(e) => setType(e.target.value)} className="bg-muted rounded-lg px-3 py-2 text-sm">
